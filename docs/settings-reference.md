...
| `agyCliEnabled` | `boolean` | `undefined` | Enables the `agy-cli` provider in model pickers after Antigravity CLI status validation. Toggle from Settings → Authentication. The Antigravity CLI (`agy`) is OAuth/session-authenticated; run `agy` once to authenticate. |
| `agyCliBinaryPath` | `string` | `undefined` | Optional global, machine-local Antigravity CLI executable override used by Settings → Authentication, status/enable validation, probes, and model discovery. Leave unset/blank to auto-detect `agy` on PATH. Use this when PATH points at the wrong `agy` install or Windows exposes a specific `.cmd`/`.bat` shim; invalid non-empty saves are rejected with bounded diagnostics. |
...
| `agy-cli` | Plugin runtime `agy` | `agy`; missing plugin names Antigravity CLI install/auth remediation | fail-fast / pinned pi fallback / assert available | drop fallback with warning |
...
Cursor selections route to the verified `cursor` plugin runtime, which runs supervised `cursor-agent --print --output-format stream-json` turns. A missing runtime still fails fast with Cursor installation and authentication remediation; a fallback-only Cursor selection is dropped with a warning rather than delegated to pi. Antigravity CLI (`agy-cli`) selections route to the `agy` plugin runtime, which runs supervised `agy --dangerously-skip-permissions --mode <accept-edits|plan> --input-format stream-json --output-format stream-json` turns; a missing runtime fails fast with Antigravity CLI install/auth remediation and a fallback-only selection is dropped with a warning. The bundled `fusion-plugin-agy-runtime` plugin is staged for explicit install (`fn plugin install fusion-plugin-agy-runtime`), not auto-installed at host startup. See the [Cursor runtime plugin README](../plugins/fusion-plugin-cursor-runtime/README.md), [Hermes README](../plugins/fusion-plugin-hermes-runtime/README.md), [Claude README](../plugins/fusion-plugin-claude-runtime/README.md), and [Antigravity CLI README](../plugins/fusion-plugin-agy-runtime/README.md) for operator installation details.
