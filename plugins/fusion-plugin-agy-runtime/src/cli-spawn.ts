...
export async function runAgyCommand(binary: string, args: string[], timeoutMs: number): Promise<{ code: number | null; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
    ...
    agy installs as a native executable (~/.local/bin/agy on macOS/Linux,
    %LOCALAPPDATA%\agy\bin on Windows) but Windows PATH may still expose a
    .cmd/.bat shim. Keep Unix/macOS on direct spawn so only the known agy
    probe/discovery seam uses shell resolution where Windows requires it.
    Streaming prompt execution deliberately diverges; see prompt-transport.ts
    for its supervised shell:false launch contract.
    */
...
export function resolveAgyBinaryForSpawn(binary: string): string {
...
// FNXC:AgyCli 2026-09-06-00:00: Bracketed agy model parameters are data, not cmd control syntax; reject only characters cmd.exe can interpret at this boundary.
const CMD_UNSAFE = /["%!^&|<>()\r\n\0]/;
export function assertCmdBoundarySafe(token: string): void {
  const offending = token.match(CMD_UNSAFE)?.[0];
  if (offending) throw new Error(`agy cmd boundary rejected token ${JSON.stringify(token)} containing ${JSON.stringify(offending)}; relocate the worktree or use a direct agy executable.`);
}
